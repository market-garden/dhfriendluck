...
function D($className='',$appName='')
{
    ...
    if('@'===$appName) {
		...
		//dump(SITE_PATH.'/apps/'.$appName.'/Lib/Model/'.$className".class.php");
		require_cache(SITE_PATH.'/apps/'.$appName.'/Lib/Model/'.$className.".class.php");
    }
    if(class_exists($className)) {
        $model = new $className();
...
function B($name,$options = null) {
	...
	if (!class_exists($class)){
		require_cache(THINK_PATH.'/../thinksns/Lib/Behavior/'.$class.'.class.php');
	}
	//修改结束
...
function W($name,$data=array(),$return=false) {
	...
	//2009-06-01修改
	if (!class_exists($class)){
		require_cache(THINK_PATH.'/../thinksns/Lib/Widget/'.$class.'.class.php');
	}
	//修改结束
