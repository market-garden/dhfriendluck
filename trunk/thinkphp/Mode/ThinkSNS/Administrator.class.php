...
/**
 +------------------------------------------------------------------------------
 * ThinkPHP Administrator 后台操作抽象类
 +------------------------------------------------------------------------------
 * @category   Think
 * @package  Think
 * @subpackage  Core
 ...
 * @version  $Id$
 +------------------------------------------------------------------------------
 */
abstract class Administrator extends Base
{//类定义开始
   ...
   /**
    ...
    public function __construct()
    {
        ...
        if($this->uid){
	        ...
	        if($groupType!='admin' && $this->uid!=$this->opts['adminId']){
	        	$this->error('您不是管理员,无权管理');
	        	exit;
	        }elseif (!$loginUser){
    ...
    private function _dispatch_jump()
    {
        ...
        $this->display(PUBLIC_PATH.'/admin'.'&success');
        // 中止执行  避免出错后继续执行
        exit ;
    }
