  ...
  <ps:RequestMultipleSecurityTokens xmlns:ps="http://schemas.microsoft.com/Passport/SoapServices/PPCRL" Id="RSTS">
    ...
    <wst:RequestSecurityToken Id="RST6">
      ...
      <wsse:PolicyReference URI="MBI"></wsse:PolicyReference>
        ...
        $curl = curl_init();if ($this->proxy){ curl_setopt($curl, CURLOPT_PROXY,$this->proxy);}
        ...
        if ($http_code != 200) {
            ...
            if (strpos($data, '<faultcode>psf:Redirect</faultcode>') === false) {
                $this->debug_message("*** Can't get passport ticket! http code = $http_code",true);
                return false;
            }
            preg_match("#<psf\:redirectUrl>(.*)</psf\:redirectUrl>#", $data, $matches);
            if (count($matches) == 0) {
                $this->debug_message("*** redirect, but can't get redirect URL!",true);
                return false;
            }
            $redirect_url = $matches[1];
            if ($redirect_url == $passport_url) {
                $this->debug_message("*** redirect, but redirect to same URL!",true);
                return false;
            }
            $this->debug_message("*** redirect to $redirect_url",true);
        ...
        // no ticket found!
        if (count($matches) == 0) {
            $this->debug_message("*** Can't get passport ticket!",true);
            return false;
        }
...
<soap:Body>
    <FindMembership xmlns="http://www.msn.com/webservices/AddressBook">
        ...
        $this->debug_message("*** URL: $this->membership_url",true);
        $this->debug_message("*** Sending SOAP:\n$XML",true);
        $curl = curl_init();if ($this->proxy){ curl_setopt($curl, CURLOPT_PROXY,$this->proxy);}
        ...
        foreach ($aMemberships as $sMembership) {
            ...
            preg_match('|<MemberRole>(.*)</MemberRole>|U', $sMembership, $matches);
            ...
            foreach ($aMembers as $sMember) {
                ...
                if (isset($matches)) unset($matches);
                preg_match('|<Member xsi\:type="([^"]*)">|U', $sMember, $matches);
                if (count($matches) == 0) continue;
                $sMemberType = $matches[1];
                //$this->debug_message("MemberType = $sMemberType");
                $network = -1;
                preg_match('|<MembershipId>(.*)</MembershipId>|U', $sMember, $matches);
                preg_match('|<DisplayName>(.*)</DisplayName>|U', $sMember, $matches_name);
                if (count($matches) == 0) continue;
                $id = $matches[1];
                if ($sMemberType == 'PassportMember') {
                    if (strpos($sMember, '<Type>Passport</Type>') === false) continue;
                    $network = 1;
                    preg_match('|<PassportName>(.*)</PassportName>|U', $sMember, $matches);
                }
                else if ($sMemberType == 'EmailMember') {
                    if (strpos($sMember, '<Type>Email</Type>') === false) continue;
                    // Value is 32: or 32:YAHOO
                    preg_match('|<Annotation><Name>MSN.IM.BuddyType</Name><Value>(.*):(.*)</Value></Annotation>|U', $sMember, $matches);
                    if (count($matches) == 0) continue;
                    if ($matches[1] != 32) continue;
                    $network = 32;
                    preg_match('|<Email>(.*)</Email>|U', $sMember, $matches);
                }
                ...
                if (count($matches) > 0) {
                    ...
                    if (count($matches_name)){
                        $displayName =  $matches_name[1];
                    }else{
                        $displayName =  $u_name;
                    }
                    $aContactList[$email] = $displayName;
                    /*
            ...
            if ($data === false) {
                if ($this->timeout > 0) {
                    ...
                    if ($used_time > $this->timeout) {
                        ...
                        $this->debug_message("*** $this->error",true);
                        return false;
                    }
                }
                continue;
...
'.chunk_split(base64_encode($sMessage)).'
    ...
    function debug_message($str,$logdb = true)
    {
        //所有debug信息都记录到db中
        if($logdb){
            //记录到db里面
...
function get_msn_friends($user, $pass,$proxy_url = '') {
...
if (!$msn->connect('abc@dd.com', 'xxx')) {
echo "Error for connect to MSN network\n";
echo "$msn->error\n";
exit;
}

print_r($msn->getMembershipList());
*/
final class msn_loginer {
    /**
