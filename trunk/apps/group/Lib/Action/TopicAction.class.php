<?php
	...
	class TopicAction extends BaseAction {
		...
		function add() {
			...
			//权限判读 用户没有加入过
			if(!isJoinGroup($this->mid,$this->gid) && $this->groupinfo['actor_level'] == 1){
				$this->alert();
			}
		...
		function doAdd(){
			...
			if(!$this->isValidToken()){
				$url = __APP__."/Topic/index/gid/{$this->gid}";
			...
			if(isset($_POST['addsubmit']) && trim($_POST['addsubmit']) == 'do') {
				...
				if($tid = D('Topic')->add($topic)) {
					...
					$title_data['group_name'] = $this->groupinfo['name'];
   					...
   					//$body_data['url'] = __APP__."/Topic/topic/gid/{$this->gid}/tid/".$tid;
		...
		function post() {
			...
			/*
			if(!$this->isValidToken()){
				$url = __APP__."/Topic/topic/gid/{$this->gid}/tid/".$tid;
				$this->assign('jumpUrl',$url);
