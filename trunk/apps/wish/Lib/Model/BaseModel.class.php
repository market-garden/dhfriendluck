<?php
    ...
    class BaseModel extends Model{
        ...
        protected function doNotice($content,$toUids){
        	if(empty($toUids)){
        		$toUids = $this->api->friend_get();
        	}
            $appid = A('Index')->getAppId();
            $title['ta'] = $this->_getUserWo($this->uid);
            $body['content'] = $content;
            $url = '{WR}/apps/wish/index.php?s=/Index/friends/uid/'.$this->uid;
            $result = $this->api->notify_send( $toUids,"wish_notice",$title,$body,$url);
            return $result;        	
        }
        
        function _getUserWo($uid){
        	$info =  $this->api->user_getInfo($uid,'sex');
        	return  $info["sex"]?'他':'她';
        }
        ...
        public function fileAway($findTime ,$condition = null){
                ...
                $test = count( array_filter(array( $start,$end )));
                if( $test !=2 ) return "error";
                $this->cTime = array( 'between', array( $start,$end ) );
                //如果查询时没有设置其他查询条件，就只是按时间来进行归档查询
                $map = $this->merge( $condition );
                //查询条件赋值
