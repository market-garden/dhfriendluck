...
$(document).ready(function(){
      $( ".input_box > textarea" ).val( "添加回复" );
        $( '#wish-coment' ).click( function(){
              $( '.jishuan' ).show();
              bq_show();
           });

    $("#wish-coment").blur(function(){
      if(!$("#wish-coment").val()){
        $( ".phiz").hide();
        $( '.jishuan').hide();
      }
...
function deleteWish( id ){
    Confirm({message:'是否删除此愿望',handler:function(button){
      if ( 'ok' == button ){
        $.post( APP+'/Index/doDeleteWish/',{id:id},function( text ){
              if( text == 1 ){
                  $( '#Fli'+id ).remove();
              }else{
                  alert( "删除失败" );
...
function insert(_this){
  ...
  var old_con = $("#wish-coment").val();
  var new_con = old_con+emotion;
  $("#wish-coment").val(new_con);
...
//  var idea = $( '#idea' ).val();
//  var who = $( '#who' ).val();
//  var age_from = $( '#f_age_from' ).val();
//  var age_to = $( '#f_age_to' ).val();
//  var area = $( '#ts_hometown' ).val();
//  var tell = $("input[@name=tell][checked='true']").val();
//  //检测合法性
//
// if(!idea) {ymPrompt.errorInfo("愿望不能为空哦~~~");return;}
//
//
...
//
//  //POST提交
//  $.post( APP+"/Index/doAddWish/",{idea:idea,who:who,age_from:age_from,age_to:age_to,area:area,tell:tell},function( txt ){
//      if(false == checkJsToken(txt)){
//          $( '.btn_big').removeAttr('disabled');
//          return true;
//      }
//      if( txt ){
//          $( '#wish-content' ).html( txt );
//          $( '#wish-time' ).html( '刚刚' );
//          $( '#wish-coment' ).val('');
//
//          $( '.btn_big').removeAttr('disabled');
...
function replay( uid,wish_id ){
  $( '.inputReplay' ).blur();
  touid = uid;
  var obj = $( '#input'+wish_id );
  replayShow(obj) ;
  if( uid != "false" ){
    old_con = obj.val();
    //通过:号分割，不能同时回复几个人
