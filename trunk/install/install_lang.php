...
$i_message['install_title'] = 'ThinkSNS1.6 beta1 安装向导';
...
$i_message['install_intro'] = '<h4>安装须知</h4><p>一、运行环境需求：PHP(5.1.2+)+MYSQL(4.1+)</p><p>二、安装步骤：<br /><br />1、使用ftp工具以二进制模式，将该软件包里的 thinksns 目录及其文件上传到您的空间，假设上传后目录仍旧为 thinksns。<br /><br />2、如果您使用的是Linux 或 Freebsd 服务器，先确认以下目录或文件属性为 (777) 可写模式。<br /><br />目录: data<br />目录: runtime<br />目录: install<br />文件: config.inc.php<br />文件: define.inc.php<br />3、运行 http://yourwebsite/thinksns/install/install.php 安装程序，填入安装相关信息与资料，完成安装！<br />4、运行 http://yourwebsite/thinksns/cleancache.php 清除系统缓存文件！<br />5、运行 http://yourwebsite/thinksns/index.php 开始体验ThinkSNS1.6！</p>';
$i_message['install_start'] = '开始安装ThinkSNS';
$i_message['install_license_title'] = '安装许可协议';
$i_message['install_license'] = '版权所有 (C) 2007-2009，ThinkSNS.com 保留所有权利。

ThinkSNS是由ThinkSNS项目组独立开发的SNS程序，基于PHP脚本和MySQL数据库。本程序源码开放的，任何人都可以从互联网上免费下载，并可以在不违反本协议规定的前提下进行使用而无需缴纳程序使用费。

官方网址： www.ThinkSNS.com 交流社区： i.ThinkSNS.com
...
$i_message['install_mysql_prefix_intro'] = '同一数据库安装多ThinkSNS程序时可改变默认值';
...
$i_message['configure_log_success'] = '配置资料写入完成';
$i_message['configure_read_failed'] = '配置文件读取错误，请检查/data/sql.inc.php属性是否为777';
$i_message['error'] = '错误';
...
$i_message['install_success_intro'] = '<p>安装程序已经顺利执行完毕，请尽快删除整个 install 目录，以免被他人恶意利用。</p><p>感谢您使用 ThinkSNS 程序.</p><p><a href="../index.php">请点击这里开始体验ThinkSNS吧！</a></p>';
$i_message['install_lock'] = 'ThinkSNS 安装程序已锁定。如要重新安装，请删除本目录的 install.lock 文件！';
$i_message['install_dbFile_error'] = '数据库配置文件无法读取，请检查/install/i_thinksns_com.sql是否存在。';
