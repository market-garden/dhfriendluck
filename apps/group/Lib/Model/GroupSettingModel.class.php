...
class GroupSettingModel extends BaseModel {
	...
	//搜索数据
	function searchData($type,$uid,$username,$title,$content,$field,$asc,$limit=null,$isDel=0){
		
		$condition = array();
		
		if($username) {
			$arr_uid = D('User')->search($username);
		...
		if($type == 'group'){
			...
			if($title) $condition[] = "name like '%".$title."%'";
   		...
   		elseif($type == 'topic'){
			...
			return 	$data;
   			...
   			if($uid) $condition[] = "uid = $uid ";
   			if($content) $condition[] = "note %'".$content."'%";
