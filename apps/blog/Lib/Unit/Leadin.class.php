<?php
    ...
    class LeadIn{
        ...
        public function deleSubscribe(array $source_id ,$uid){
            ...
            if( 0 == $liveNum){
                //删除组件
                $delete = D( "BlogItem" )->deleteItem( $map );
                unset( $map );
        ...
        private function _update( $data,$source_id ){
            ...
            //取得数据库中已有的链接
            $map['sourceId'] = $source_id;
            $item_links      = D( 'BlogItem' )->getItem( $map,'link' );
            ...
            $change_ids = D( 'BlogItem' )->addItem( $data,$links,$source_id );
        ...
        private function _factory( $classname ){
           return D( "Blog".ucfirst( $classname ) );
        }
