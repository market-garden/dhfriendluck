<?php
    ...
    class AdminAction extends Administrator{
        ...
        public function wishlist (){
            //姓名，uid,愿望内容
            $_POST['name']     && $this->wish->name    = t( $_POST['name'] );
            $_POST['uid']      && $this->wish->uid     = intval( t( $_POST['uid'] ) );
            $_POST['content']  && $this->wish->content = array( 'like',"%".t( $_POST['content'] )."%" );

            //处理时间
            $_POST['stime'] && $_POST['etime'] && $this->wish->cTime = $this->wish->DateToTimeStemp(t( $_POST['stime'] ),t( $_POST['etime'] ) );

            //处理排序过程
            $order = t( $_POST['sorder'] )." ".t( $_POST['eorder'] );
            $list  = $this->wish->getWishList(null,null,$order,t($_POST['limit']));
            $this->assign( $_POST );
            $this->assign( $list );
            $this->display();
        }
        ...
        public function doDeleteWish(){
            $wishid['id'] = array( 'in',$_REQUEST['id']);        //要删除的id.
            $result       = $this->wish->doDeleteWish($wishid);
            if( false !== $result){
