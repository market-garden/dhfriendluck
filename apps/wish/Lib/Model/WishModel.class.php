<?php
    ...
    class WishModel extends BaseModel {
         ...
         * @return void
         */
        public function getLastWish ( $userId, $options = null) {
            //过滤参数
            if( is_array( $options ) ){
                $temp    = $options;
         ...
         * @return object|array
         */
        public function getWishList($map = null,$field=null,$order = null,$limit = null) {
            //处理where条件
            $map = $this->merge( $map );
            ...
            foreach( $data as &$value ){
                if( $value['uid'] == $this->uid ) $value['isDelete'] = true;
                if($value['type']=='2'){
         ...
         * @return void
         */
        public function doDeleteWish( $map = null ) {
            //获得配置信息
            ...
            $result = $this->api->space_changeCount( 'wish',$count );
            return $result;
        }
         ...
         * @return void
         */
        public function getMyReplyWish( $appid ){
            $condition['id'] = array( 'in',$appid );
            $condition['status'] = 0;
            $result = $this->getWishList( $condition,'*','cTime DESC' );
            return $result;
        }
        ...
        public function getMyReplyId($condition){
            ...
            $myReplyWish = $comment->where( $map )->field( 'distinct(appid)' )->findAll();

            $appid       =  array();
            foreach( $myReplyWish as $value ){
                $appid[] = $value['appid'];
            }
            return $appid;
        }
        ...
        public function doAddWish ($map) {
            ...
            //发送通知
            if($tell){
            	$this->doNotice($map['content']);
            }
/*             setScore($map['uid'],'add_wish');
            ...
            //发出动态，并更新feedId
            $map2    = $this->doFeed("wish",$title,$body_data);
            if( false != $map2 ){
        ...
        public function ChangeCount($id,$count = null){
          $sql = "UPDATE {$this->tablePrefix}wish
